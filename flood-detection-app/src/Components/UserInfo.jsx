...
export default function Userinfo({ navigation }) {
  ...
  const getToken = async () => {
    try {
      ...
      setFcmToken(token); // Update the state with the FCM token
      console.log('FCM Token:', token);
    } catch (error) {
      console.error('Error getting FCM token:', error);
      // Handle token retrieval errors, e.g., display error message to user
    }
  };
  ...
  const handleTextInput = async () => {
    try {
      ...
      // Push the user data to the users node and get the generated key
      const newUserRef = usersRef.push(userData);

      // Get the unique key generated by Firebase for the new user
      const userkey = newUserRef.key;
      await AsyncStorage.setItem('id', userkey);

      console.log('User added successfully with ID:', userkey);

      // Save the data to AsyncStorage after setting the id state
      handleSaveData();
